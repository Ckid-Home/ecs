...
TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/nexttrace.XXXXXX") || exit 1
...
"i386" | "i686")
  ...
  ;;
*)
  _red " 本脚本只支持 AMD64、ARM64、i386 和 i686 使用，问题反馈:[https://github.com/fscarmen/tools/issues] " && exit 1
  ;;
esac
...
# 下载 nexttrace 主程序
if [ ! -e "$FILE_PATH" ]; then
  VERSION=$(curl -sSL --proto '=https' --proto-redir '=https' "https://api.github.com/repos/nxtrace/Ntrace-core/releases/latest" | awk -F \" '/tag_name/{print $4}')
  [[ "$VERSION" =~ ^[A-Za-z0-9._-]+$ ]] || exit 1
  curl --fail -sSL --proto '=https' --proto-redir '=https' "https://github.com/nxtrace/Ntrace-core/releases/download/$VERSION/$FILE" -o "$FILE_PATH" || exit 1
  chmod 700 "$FILE_PATH" >/dev/null 2>&1
fi

# 查路由
RESULT=$("$FILE_PATH" "$ip" -g cn 2>/dev/null)

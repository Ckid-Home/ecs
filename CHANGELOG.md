...
- 修复文件不存在时错误截断文件导致意外创建空文件的问题
- 修复 DD 磁盘测试中双重重定向导致缓存清除失败的问题
- 修复 DD 磁盘测试写入速度单位转换正则无效导致单位残留中文的问题
- 修复 DD 磁盘测试写入 IOPS 在最终输出行缺少 K 单位后缀的问题
- 修复 DD 磁盘测试写入时间为空时 IOPS 计算发生除零崩溃的问题
- 修复 DD 磁盘测试读取时间为空时 IOPS 计算发生除零崩溃及缺少 K 单位的问题
- 修复 IPv6 ASN 信息清理时错误使用 IPv4 变量导致数据残留的问题
- 修复 BBR 内核优化开关逻辑反转导致确认后跳过优化的问题
- 修复 IPv6 私有地址检测将所有 2001: 开头的地址误判为 Teredo 私有地址的问题
- 修复自定义 IP 质量检测中 VirusTotal 密钥列表为空时发生除零崩溃的问题
- 修复自定义 IP 质量检测在获取 root 权限前即执行更新导致权限不足的问题
- 修复磁盘信息检测中 hd* 检测逻辑意外混入 nvme* 设备的问题
- 修复磁盘信息检测中 vd* 检测逻辑意外混入 mmcblk* 设备的问题
- 修复 i386/i686 架构下回程路由测试错误使用 macOS 二进制文件的问题
- 修复 macOS 版本检测命令拼写错误导致回程路由测试在 macOS 上始终失败的问题

...
MY_GEEKBENCH_EMAIL=""
MY_GEEKBENCH_KEY=""
RUN_TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/geekbench6.XXXXXX") || exit 1
...
if curl --fail --location --proto '=https' --proto-redir '=https' "$MY_GEEKBENCH_DOWNLOAD_URL" -o "$ARCHIVE_FILE" 2>/dev/null; then
	...
	if ! tar tzf "$ARCHIVE_FILE" | grep -Eq '(^|/)\.\.(/|$)|^/' &&
		...
		if [[ -f "$MY_DIR/geekbench6" && ! -L "$MY_DIR/geekbench6" && -x "$MY_DIR/geekbench6" ]]; then
...
if [[ $MY_GEEKBENCH_NO_UPLOAD ]]; then
	"$MY_DIR/geekbench6" --no-upload >>"$MY_OUTPUT" 2>&1 || exit_with_failure "Could not run Geekbench"
else
	"$MY_DIR/geekbench6" --upload >>"$MY_OUTPUT" 2>&1 || exit_with_failure "Could not run Geekbench"
fi
# cat "$MY_OUTPUT"
GEEKBENCH_URL=$(grep -o 'https://browser.geekbench.com/v6/cpu/[0-9]\+' "$MY_OUTPUT" | head -n1)
